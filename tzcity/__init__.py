...
def capsify(name: str) -> str:
    ...
    LOWER_ONLY = ['la', 'de', 'da', 'and', 'of', 'the', 'es']

    # For 'UK', 'UAE', etc
    UPPER_ONLY = ['uk', 'uae', 'sgssi']

    # For 'Port-au-Prince', 'Fort-de-France', etc
    HYPHENATED = ['au', 'de']

    # For 'McMurdo', 'Dumont d'Urville', 'N'Djamena', etc
    OTHERS = {'mc': 'Mc', "d'": "d'", "n'": "N'"}   #length of each value must be same as its key.
    ...
    for word in words:
        new_word: str = ''
        if word in LOWER_ONLY:
            new_word = word.lower() #REDUNDANT
        elif word in UPPER_ONLY:
            new_word = word.upper()
        elif any([f"-{x}-" in word for x in HYPHENATED]):
            ...
            if re_match is not None:
                ...
                if len(word) > len(match_str):
                    ...
                    raise ValueError(f"Could not capitalize '{word}'. Unknown pattern")
            else:
                new_word = word.title()
